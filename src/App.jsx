...
const colors = {
  bg: "#0a0a0b",
  text: "#e8ecef",        // gris clair (texte & dates)
  muted: "#c2c7cc",
  panel: "#141414",
  border: "#1f2021",
  axis: "#8a8f94",
...
function useDashboardData() {
  ...
  const [userTrades] = useState([]) // branchement futur CSV/API
  ...
  const mtd = useMemo(()=>{
    return filtered.filter(t=>inMonth(t.date, yNow, mNow)).reduce((s,t)=>s+convert(t.pnl,t.ccy,displayCcy),0)
  }, [filtered, displayCcy, rates])
  const ytd = useMemo(()=>{
    return filtered.filter(t=>(+t.date.slice(0,4)===yNow)).reduce((s,t)=>s+convert(t.pnl,t.ccy,displayCcy),0)
  }, [filtered, displayCcy, rates])
  const weekdays = useMemo(()=>{
    ...
    const keys = ['Lun','Mar','Mer','Jeu','Ven','Sam','Dim']
  ...
  // MAE/MFE (agrégés simples pour démo)
  const maeAgg = useMemo(()=>{
    const pos = filtered.reduce((a,t)=>a+(t.mae?Math.abs(convert(t.mae,t.ccy||'USD',displayCcy)):0),0)
    return { total: fix2(pos), n: filtered.length }
  }, [filtered, displayCcy, rates])
  const mfeAgg = useMemo(()=>{
    const pos = filtered.reduce((a,t)=>a+(t.mfe?Math.abs(convert(t.mfe,t.ccy||'USD',displayCcy)):0),0)
    return { total: fix2(pos), n: filtered.length }
  }, [filtered, displayCcy, rates])
...
const hourlyAlerts = useMemo(()=>{
  ...
  return {
    ...
    // devise
    displayCcy, setDisplayCcy, convert, fmt,
    // données calculées
    capitalInitialDisp, cashflowsInRange, pnlByDate, equityWithFlowsSeries,
    maxDDAbs, maxDDPct,
    totalPnlDisp, cashFlowTotal, capitalBase, capitalGlobal, globalReturnPct,
    wr, rr, expectancy, avgWin, avgLoss, sharpe, sortino,
  ...
  return (
    <div style={{ ...baseCard, display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 10 }}>
      <div><div style={label}>Actif</div>
        <select value={asset} onChange={e => setAsset(e.target.value)} style={sel}><option>All</option>{assets.map(a => <option key={a}>{a}</option>)}</select>
      </div>
      <div><div style={label}>Broker</div>
        <select value={broker} onChange={e => setBroker(e.target.value)} style={sel}><option>All</option>{brokers.map(b => <option key={b}>{b}</option>)}</select>
      </div>
      <div><div style={label}>Stratégie</div>
        <select value={strategy} onChange={e => setStrategy(e.target.value)} style={sel}><option>All</option>{strategies.map(s => <option key={s}>{s}</option>)}</select>
      </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12, marginTop: 12 }}>
      ...
      <div style={card(ddTone(maxDDPct))}>
        ...
        <div style={{ fontSize:18 }}>
          ...
          {' / '}
          <span style={{ color: colors.pink }}>{fmtLocal(Math.abs(maxDDAbs))}</span>
        </div>
      </div>
    </div>
  )
...
/* ===================== BLOC 4 — KPIs RATIOS ===================== */
function KPIRatiosSection({
  wr, rr, expectancy, avgWin, avgLoss,
  sharpe, sortino, avgWinDurMin, avgLossDurMin,
  mtd, ytd, weekdays
}) {
  const profitable = (() => {
    const p = isFinite(wr) && wr>0 ? (wr/100) : 0
    return (avgWin * p - Math.abs(avgLoss) * (1 - p)) > 0
  })()
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12, marginTop: 12 }}>
      ...
      <div style={card(profitable ? toneBySign(avgWin-avgLoss) : 'red')}>
        <h3 style={title}>Gain Moyen / Perte Moyenne</h3>
        <div style={{ fontSize:16 }}>
          <span style={{ color: colors.green }}>{fmtLocal(avgWin)}</span>
          {' / '}
          <span style={{ color: colors.pink }}>{fmtLocal(avgLoss)}</span>
        </div>
      </div>
      <div style={card(null)}>
...
/* ===================== BLOC 5 — COURBE D'ÉQUITÉ ===================== */
function EquitySection({ equityWithFlowsSeries, cashflowsInRange }) {
  return (
    <div style={{ ...baseCard, height: 420, marginTop: 16 }}>
      ...
      <ResponsiveContainer width="100%" height="88%">
        <LineChart data={equityWithFlowsSeries} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
          ...
          <Tooltip content={<EquityTooltip colors={colors} cashflows={cashflowsInRange} />} />
...
/* ===================== BLOC 6 — GAINS/PERTES PAR HEURE/JOUR/MOIS ===================== */
function GainsLossSection({ gainsLossByHour, gainsLossByDay, gainsLossByMonth }) {
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12, marginTop:16 }}>
      <BarCard title="Gains / Pertes Par Heure D’Ouverture" data={gainsLossByHour} xKey="hour" />
      <BarCard title="Gains / Pertes Par Jour D’Ouverture"  data={gainsLossByDay}  xKey="day"  />
      <BarCard title="Gains / Pertes Par Mois D’Ouverture"  data={gainsLossByMonth} xKey="month" />
    </div>
  )
}

/* ===================== BLOC 7 — MAE / MFE ===================== */
function MaeMfeSection({ maeAgg, mfeAgg }) {
  return (
    <div style={{ ...baseCard, marginTop:16 }}>
      <h3 style={title}>MAE / MFE (Agrégés Démo)</h3>
      <div style={{ display:'flex', gap:24, fontSize:16 }}>
        <div>MAE Total: <span style={{ color: colors.pink }}>{fmtLocal(maeAgg.total)}</span> — {maeAgg.n} trades</div>
        <div>MFE Total: <span style={{ color: colors.green }}>{fmtLocal(mfeAgg.total)}</span> — {mfeAgg.n} trades</div>
      </div>
      <div style={{ color: colors.muted, fontSize:12, marginTop:8 }}>
        * Démo agrégée — on pourra passer à des graphiques (scatter/courbes) par trade si tu veux.
      </div>
    </div>
  )
}
...
function EquityTooltip({ active, payload, label, colors, cashflows }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 6 }}>Équité: <b style={{ color: '#fff' }}>{fmtLocal(equity)}</b></div>
      {flows.length>0 && (
        <div>
          {flows.map((c,i)=>(
            <div key={i} style={{ color: c.amount>=0?colors.green:colors.pink }}>
              {(c.type==='deposit' && 'Dépôt') ||
               ...
               (c.type==='darwin_mgmt_fee' && 'Darwinex Fee') || 'Flux'}: {c.amount>=0?'+':''}{fmtLocal(c.amount_disp)}
            </div>
          ))}
        </div>
      )}
...
function BarCard({ title, data, xKey }) {
  return (
      ...
      <ResponsiveContainer width="100%" height="88%">
        <BarChart data={data} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
          ...
          <Tooltip
            ...
            formatter={(v, n)=>[fmtLocal(v,true), n==='gain'?'Gains':'Pertes']}
          />
...
function Value({ v, currency=false, percent=false }) {
  const color = percent ? numColor(v) : numColor(v)
  const text = percent ? `${v.toFixed(2)}%` : fmtLocal(v)
  return <div style={{ fontSize:18, color }}>{text}</div>
}

// format local simple (sans connaître displayCcy ici)
function fmtLocal(v, compact=false){
  const n = (v??0)
  if (compact) return n.toLocaleString(undefined,{ minimumFractionDigits:2, maximumFractionDigits:2 })
  ...
  return (
    <div style={{ minHeight: "100vh", background: colors.bg, color: colors.text, padding: 20, maxWidth: 1540, margin: "0 auto" }}>
      ...
      <HeaderSection
        ...
        setDisplayCcy={m.setDisplayCcy}
      />

      {/* Bloc 2 */}
      <FiltersSection
        assets={m.assets} brokers={m.brokers} strategies={m.strategies}
        asset={m.asset} setAsset={m.setAsset}
        broker={m.broker} setBroker={m.setBroker}
        strategy={m.strategy} setStrategy={m.setStrategy}
        dateFrom={m.dateFrom} setDateFrom={m.setDateFrom}
        dateTo={m.dateTo} setDateTo={m.setDateTo}
      />

      {/* Bloc 3 */}
      <KPICoreSection
        capitalInitialDisp={m.capitalInitialDisp}
        cashFlowTotal={m.cashFlowTotal}
        totalPnlDisp={m.totalPnlDisp}
        capitalGlobal={m.capitalGlobal}
        globalReturnPct={m.globalReturnPct}
        maxDDPct={m.maxDDPct}
        maxDDAbs={m.maxDDAbs}
      />
      ...
      {/* Bloc 4 */}
      <KPIRatiosSection
        wr={m.wr} rr={m.rr} expectancy={m.expectancy}
        avgWin={m.avgWin} avgLoss={m.avgLoss}
        sharpe={m.sharpe} sortino={m.sortino}
        avgWinDurMin={m.avgWinDurMin} avgLossDurMin={m.avgLossDurMin}
        mtd={m.mtd} ytd={m.ytd} weekdays={m.weekdays}
      />

      {/* Bloc 5 */}
      <EquitySection
        equityWithFlowsSeries={m.equityWithFlowsSeries}
        cashflowsInRange={m.cashflowsInRange}
      />

      {/* Bloc 6 */}
      <GainsLossSection
        gainsLossByHour={m.gainsLossByHour}
        gainsLossByDay={m.gainsLossByDay}
        gainsLossByMonth={m.gainsLossByMonth}
      />

      {/* Bloc 7 */}
      <MaeMfeSection maeAgg={m.maeAgg} mfeAgg={m.mfeAgg} />
