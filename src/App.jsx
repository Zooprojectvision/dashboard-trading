...
function GuidePanel({ lang }) {
  ...
  return (
    <>
      ...
      >
        {(dict[lang]?.actions?.Help) ||
          I18N_DEFAULTS.actions.Help}
      </button>
...
function ActivityBlocks({ rows }) {
  ...
  rows.forEach(t => {
    // pas d'heure précise dans CSV -> heure random
...
function CalendarMonthly({
  ...
  for (
    ...
    // calcul équité locale (DD indicatif)
    const prev = eq;
    eq += isInMonth
      ? dayData.pnl
      : 0;
...
function AppFooter() {
  return (
    ...
    >
      Designed &amp; Built by ZooProjectVision V
      {APP_VERSION} @{' '}
...
export default function App() {
  // --- Navigation simple ---
  const [view, setView] =
    React.useState('home'); // 'home' | 'control' | 'compta' | 'risk'
  ...
  React.useEffect(() => {
    ...
    fetch(
      'https://api.exchangerate.host/latest?base=USD&symbols=EUR,CHF'
    )
      .then(r => r.json())
      .then(j => {
        const data = {
